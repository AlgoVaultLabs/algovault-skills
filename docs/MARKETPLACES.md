...
| 2 | [Smithery Registry](https://smithery.ai/skills/algovault) | **LIVE + CI auto-republish working** | Published 2026-04-24 as 20 individual skills under the `algovault` namespace (Smithery CLI v4.x is per-skill, not bundle). Browse: <https://smithery.ai/skills/algovault>. Install one with `npx -y @smithery/cli skill add algovault/<slug>`. CI auto-republishes on every `git tag v*` via `.github/workflows/publish.yml` (loops over `skills/manifest.json`); verified GREEN 20/20 in run [25063072849](https://github.com/AlgoVaultLabs/algovault-skills/actions/runs/25063072849). **Token-rotation runbook:** the `SMITHERY_API_KEY` repo secret holds a Smithery **session token** (~549 bytes, NOT a service token from `auth token` which is restricted to MCP-server access and does NOT grant `skill publish`). To rotate: `npx -y @smithery/cli auth login` (browser OAuth) → `printf '%s' $(npx -y @smithery/cli auth whoami --full \| awk '/^Token:/{print $2}') \| gh secret set SMITHERY_API_KEY --repo AlgoVaultLabs/algovault-skills` (omit `--body` so gh reads stdin — passing `--body -` is a 1-byte literal "-", not stdin). |
